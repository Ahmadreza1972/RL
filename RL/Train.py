...
from anylogic-alpyne.sim import AnyLogicSim
...
if __name__ == "__main__":
    ...
    sim = AnyLogicSim(r"Maze_LR Exported/model.jar", engine_overrides=dict(seed=147))
    print(sim.schema)
    print("---------")
    ...
    num_hole=0
    num_goal=0
    matrix=pd.array(pd.read_csv("Maze_LR\matrix.csv"))
    for i in 60:
        for j in 60:
            if matrix[i][j]==1:
                num_walls+=1
                ...
                num_goal+=1
    config = dict(slipChance=0.0,num_goal=num_goal,num_wall=num_walls,num_hole=num_hole,matrix=matrix)
